...
import {p2pProps} from 'store/wallet/types';
...
const CoinCeFIItem = ({coin, onPress}: CoinCeFIItemT) => {
      ...
      <View style={[styles.row, {flex: 1}]}>
        {coin?.coin?.image ? (
          <View>
            ...
            {coin.networks[0]?.network?.network !== coin.coin.symbol &&
              !Object.values(HIDE_SUB_ICON).includes(coin.coin.name) && (
                <View style={styles.subIcon}>
                  <CustomImage
                    svgSize="12"
                    image={`https://cdn.fintopio.com/${coin.networks[0]?.network?.network}.svg`}
                  />
                </View>
              )}
          </View>
        ...
        <NetworkItem network={coin.networks[0]?.network?.network} />
      </View>
      <View style={[styles.row, {justifyContent: 'flex-end', flex: 2.5}]}>
        <View style={styles.rowBalance}>
          <CustomText fontS={14} fontW="400" color={theme.colors.primary800}>
            {isHideWithNoBlur && isHideBalance ? '*****' : coin.available}
          </CustomText>
          <CustomText
            style={styles.text}
            fontS={10}
        ...
        <TouchableOpacity
          activeOpacity={0.8}
          hitSlop={HIT_SLOP}
          onPress={() => copyToClipboard(coin.networks[0]?.address)}>
          <CopyIcon />
        </TouchableOpacity>
      </View>
    </Pressable>
